"""
Extract slot / assignment tensors from a POMOSlot checkpoint and dump them to a
JSON file that the slot-collapse dashboard (index.html) can render.

This mirrors POMOSlot.shared_step data plumbing so the extracted tensors are the
exact ones the metric loss sees: slots (B, K, d) and A_ik (B, N_cust, K) over the
customer nodes (node 0 treated as a pseudo-depot, excluded from A_ik).

Usage (on the machine that trained the model, e.g. Windows):
    python scripts/slot_collapse_viz/extract_slots.py \
        --ckpt logs/train/runs/tsp50/am-tsp50/<run>/checkpoints/epoch_099.ckpt \
        --num_locs 50 --num_slots 8 --out slot_viz_data.json \
        [--batch 0] [--device cuda]

Output: a JSON with keys:
    locs    (N_cust, 2)            customer node coordinates  [node 0 excluded]
    slots   (K, d)                 slot embeddings (average over batch-dim, or batch 0)
    A       (N_cust, K)            soft assignment, batch 0
    D       (K, K)                 pairwise slot-embedding distance (cdist)
    usage   (K,)                   per-slot marginal usage = A.mean(0)
    diag    { min_offdiag, max_offdiag, mean_offdiag, norm_min, entropy, perplexity, eff_slots }
"""
...
def extract(model, locs) -> dict:
    """Run one forward pass to populate the encoder side-channel, then read slots/A_ik."""
